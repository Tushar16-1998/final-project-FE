// import React from 'react'
// import Navbar from './NavBar'

// function About() {
//     return (
//         <div>
//             <Navbar />
//             <section>
//             <h3>RENTASSIST</h3>
//             <p></p>Contact us for more information!<br/>
//             <div>
//                 <article>
//                     <p>Daniel Amaral</p>
//                     <div class="buttondan">
//                     <button class="button">
//                         <a href="https://www.linkedin.com/in/daniel-amaral-6b077445/" class="no-link-style" target="_blank">
//                         <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/81/LinkedIn_icon.svg/2048px-LinkedIn_icon.svg.png" width="65" height="65"   transform="matrix(1, 0, 0, 1, -7.105427357601002e-15, -3.552713678800501e-15)" />
//                     </a></button>

//                     <button class="button">
//                         <a href="https://github.com/dfbamaral" class="no-link-style" target="_blank">
//                         <img src="https://cdn-icons-png.flaticon.com/512/25/25231.png" width="65" height="65"   transform="matrix(1, 0, 0, 1, -7.105427357601002e-15, -3.552713678800501e-15)" />
//                     </a></button>

//                     </div>

//                 </article>
//                 <article>
//                     <p>Tushar Sunder</p>
//                     <div class="buttonmig">
//                     <button class="button">
//                         <a href="https://www.linkedin.com/in/tushar-sunder-06b6492b4/" class="no-link-style" target="_blank">
//                         <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/81/LinkedIn_icon.svg/2048px-LinkedIn_icon.svg.png" width="65" height="65"   transform="matrix(1, 0, 0, 1, -7.105427357601002e-15, -3.552713678800501e-15)" />
//                     </a></button>

//                     <button class="button">
//                         <a href="https://github.com/Tushar16-1998/" class="no-link-style" target="_blank">
//                         <img src="https://cdn-icons-png.flaticon.com/512/25/25231.png" width="65" height="65"   transform="matrix(1, 0, 0, 1, -7.105427357601002e-15, -3.552713678800501e-15)" />
//                     </a></button>

//                     </div>


//                 </article>
//                 <article>
//                         <p></p>
//                         <p></p>
//                         <p></p>
//                         <p></p>
//                         <p></p>Github Repo.<br/>
//                         <a href="https://github.com/Tushar16-1998/final-project-FE" target="_blank">
//                         <button className="Btn">
//                         <svg className="svgIcon" viewBox="0 0 496 512" height="1.4em" xmlns="http://www.w3.org/2000/svg"><path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z"></path></svg>
//                         <span className="text">Github</span>
//                         </button>
//                         </a>
//                 </article>
//             </div>
//         </section>

//         </div>

//     )
// }

// export default About


// About.js
import React from 'react';
import Navbar from './NavBar';
import 'tailwindcss/tailwind.css'; // Import Tailwind CSS styles

function About() {
    return (
        <div className="bg-gradient-to-b from-teal-500 to-blue-800 text-white">
            <Navbar />
            <section className="p-4">
                <h3 className="text-3xl font-bold">RENT ASSIST</h3>
                <p>Contact us for more information!</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <article className="bg-white p-4 rounded shadow-md">
                        <p className="text-xl text-color-black font-semibold">Daniel Amaral</p>
                        <div className="flex mt-2 space-x-2">
                            <button className="btn btn-primary">
                                <a href="https://www.linkedin.com/in/daniel-amaral-6b077445/" target="_blank">
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/81/LinkedIn_icon.svg/2048px-LinkedIn_icon.svg.png" alt="LinkedIn" className="w-8 h-8" />
                                </a>
                            </button>

                            <button className="btn btn-primary">
                                <a href="https://github.com/dfbamaral" target="_blank">
                                    <img src="https://cdn-icons-png.flaticon.com/512/25/25231.png" alt="GitHub" className="w-8 h-8" />
                                </a>
                            </button>
                        </div>
                    </article>
                    {/* Other articles go here */}
                </div>
                <p className="mt-8">Github Repo.</p>
                <a href="https://github.com/Tushar16-1998/final-project-FE" target="_blank" className="btn btn-primary mt-2">
                    <svg className="w-4 h-4 mr-2" viewBox="0 0 496 512" xmlns="http://www.w3.org/2000/svg">
                        {/* GitHub icon */}
                    </svg>
                    Github
                </a>
            </section>
        </div>
    );
}

export default About;
